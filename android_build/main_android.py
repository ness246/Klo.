...
# Android için dinamik ekran boyutu
# Varsayılan: 540x900 (3:5 oranı) - Android'de tam ekran olacak
import os
# Android ortamında tam ekran kullan
ANDROID_ARG = 'ANDROID_ARGUMENT' in os.environ or hasattr(os, 'system') and 'ANDROID' in str(os.system('echo $ANDROID_ARGUMENT'))
if ANDROID_ARG:
    # Android'de pygame display info'dan ekran boyutunu al
    # İlk pygame.init() sonrası ekran boyutunu alacağız
    SCREEN_W, SCREEN_H = 540, 900  # İlk değer, daha sonra güncellenecek
else:
    # Test için: Masaüstü ortamı
    SCREEN_W, SCREEN_H = 540, 900

# Android için ölçekleme faktörü (dokunmatik ekranlar için)
ANDROID_SCALE = max(SCREEN_W / 540, SCREEN_H / 900)
MARGIN = int(18 * ANDROID_SCALE)
...
# Android için dokunmatik ayarları - daha yüksek threshold
DRAG_THRESHOLD = int(12 * ANDROID_SCALE)  # Dokunmatik için daha yüksek
HITBOX_PADDING = int(12 * ANDROID_SCALE)  # Dokunmatik için daha büyük hitbox
...
def get_text(key):
    translations = {
        "TR": {
            ...
            "howto_touch": "Dokunmatik: Blokları sürükleyin",
            "howto_back": "Geri",
            "game_moves": "Hamle",
            "game_time": "Süre",
            "game_exit": "EXIT",
        ...
        "EN": {
            ...
            "howto_touch": "Touch: Drag blocks to move",
            "howto_back": "Back",
            "game_moves": "Moves",
            "game_time": "Time",
            "game_exit": "EXIT",
...
# Pygame screen & fonts
# -------------------
# Android için tam ekran ve landscape-y portrait desteği
import sys
ANDROID_MODE = 'ANDROID_ARGUMENT' in os.environ

if ANDROID_MODE:
    # Android: Tam ekran, dokunmatik optimizasyonlu
    # Android'de gerçek ekran boyutunu al
    display_info = pygame.display.Info()
    try:
        # Android'de tam ekran boyutunu kullan
        SCREEN_W = display_info.current_w if hasattr(display_info, 'current_w') else 540
        SCREEN_H = display_info.current_h if hasattr(display_info, 'current_h') else 900
    except:
        SCREEN_W, SCREEN_H = 540, 900
    
    window = pygame.display.set_mode((SCREEN_W, SCREEN_H), pygame.FULLSCREEN | pygame.SCALED)
    screen = pygame.Surface((540, 900))  # Sabit internal resolution
    scale_x = SCREEN_W / 540.0
    scale_y = SCREEN_H / 900.0
    scale = min(scale_x, scale_y)
else:
    # Test için: Normal pencere
...
# Android için dokunmatik koordinat dönüştürme
def transform_mouse_pos(pos):
    """Android dokunmatik koordinatlarını internal resolution'a çevirir"""
    mx, my = pos
    if ANDROID_MODE:
        # Android'de scale faktörü ile dönüştür
        transformed_x = int(mx / scale_x)
        transformed_y = int(my / scale_y)
        return transformed_x, transformed_y
    else:
        return pos

# Android için ekran güncelleme
def present():
    if ANDROID_MODE:
        # Android'de screen'i window'a ölçekleyerek çiz
        scaled = pygame.transform.scale(screen, (SCREEN_W, SCREEN_H))
        window.blit(scaled, (0, 0))
        pygame.display.flip()
    else:
        # Normal modda doğrudan aktar
...
while running:
    ...
    for ev in pygame.event.get():
        ...
        if game.state == "menu":
            ...
            # Android: F11 tuşu yok, dokunmatik ekranda zaten tam ekran
            continue
        ...
        # Nasıl Oynanır state kontrolü
            ...
            # Android: ESC tuşu Android geri butonuna eşit
            if ev.key == K_ESCAPE or (ANDROID_MODE and ev.type == pygame.QUIT):
    ...
    if game.state == "howto":
        ...
        current_y += line_spacing
        draw_text(screen, get_text("howto_touch"), (SCREEN_W//2 - 200, current_y), font)
